...
export async function parseCreativePreferences(message) {
  const structuredPreferences = createStructuredPreferences(message);

  if (structuredPreferences) {
...
function createFallbackPreferences(message) {
  ...
  const tone = extractField(message, "tone");
  const audience = extractField(message, "audience");
  const goal = extractField(message, "goal");

  return {
    tone: tone || pickFirst(lower, [
      "funny",
      "premium",
      "chaotic",
      "educational",
    ...
    audience: audience || inferAudience(lower),
    goal: goal || pickFirst(lower, [
      "app installs",
      "purchases",
      "signups",
      "clicks",
...
function createStructuredPreferences(message) {
  const tone = extractField(message, "tone");
  const audience = extractField(message, "audience");
  const goal = extractField(message, "goal");

  if (!tone || !audience || !goal) {
    return null;
  }
