...
import { pickAssets, warmAssetCache } from "@/lib/assetPicker";
import { analyzeProductUrl } from "@/lib/productAnalyzer";
import { parseCreativePreferences } from "@/lib/preferenceParser";
import { createPreferenceOptions } from "@/lib/preferenceOptions";
import { renderUgcVideo, warmRenderBundle } from "@/lib/renderVideo";
import { createVideoRecipe } from "@/lib/videoRecipe";
import { updateAssetHistory } from "@/lib/assetMemory";

const preferenceQuestions =
  "Pick the website-specific roast angle below. Every option is intentionally sarcastic, so choose the one that feels most like the product's audience.";
...
export async function createAssistantReply(messages, context = {}) {
  ...
  if (context.step === "needs_product_description" && context.productUrl) {
    ...
    const preferenceOptions = createPreferenceOptions(productBrief);
    prewarmGeneration({
      productBrief,
      pageFacts: {
        ogImage: ""
...
async function analyzeUrlReply(url) {
  try {
    ...
    const preferenceOptions = createPreferenceOptions(brief);
    prewarmGeneration({
      productBrief: brief,
...
function prewarmGeneration({ productBrief, pageFacts }) {
  warmAssetCache({ productBrief, pageFacts });
  warmRenderBundle();
}
