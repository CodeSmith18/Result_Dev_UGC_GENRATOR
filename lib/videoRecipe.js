...
- The GIF query should be product-specific and target a big cutout human meme sticker.
...
function createFallbackVideoRecipe({ productBrief, preferences }) {
  ...
  if (isPremium) {
    return normalizeRecipe({
      ...
      gifQuery: `impressed reaction sticker ${productBrief.category}`,
  ...
  if (isEducational || isFounder) {
    return normalizeRecipe({
      ...
      gifQuery: `explaining pointing reaction sticker ${productBrief.category}`,
      backgroundQuery: hints.backgroundQuery,
